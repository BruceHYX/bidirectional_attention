...
def SelectTokens(data,labels,word2idx):
    ...
    for i,sample in enumerate(data):
        for token in sample:
            if labels[i] == 1:
                if idx2word[token] in pos_num.keys():
                    pos_num[idx2word[token]] += 1
                else:
                    pos_num[idx2word[token]] = 1
            else:
                if idx2word[token] in neg_num.keys():
                    neg_num[idx2word[token]] += 1
                else:
                    neg_num[idx2word[token]] = 1
    ...
    pos_keys = pos_num.keys()
    neg_keys = neg_num.keys()
    for word in word2idx.keys():
        if word in pos_keys:
            pos_freq = pos_num[word]
        else:
            pos_freq = 0

        if word in neg_keys:
            neg_freq = neg_num[word]
        else:
            pos_freq = 0

        crit = (pos_freq-neg_freq) / (pos_freq+neg_freq) * 1.0

        if crit > 0.5 and pos_freq>5:
            pos_tokens.append(word)
        elif crit < -0.5 and neg_freq>5:
            neg_tokens.append(word)
        elif abs(crit) < 0.1 and abs(pos_freq-neg_freq)<5:
...
def GradientImportance(model,test_batch,test_labels,mark):
    ...
    plt.hist(pos_kendall,edgecolor='black',label='pos',density=True,bins=20,alpha=0.5,range=(-1,1))
    plt.hist(neg_kendall,edgecolor='black',label='neg',density=True,bins=20,alpha=0.5,range=(-1,1))
    plt.legend()
    if mark.lower() == 'softmax':
